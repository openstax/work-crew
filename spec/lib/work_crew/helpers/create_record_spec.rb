...
RSpec.describe 'WorkCrew::Helpers.create_record' do
  let(:target_group_uuid)          { SecureRandom.uuid.to_s }
  let(:target_group_desc)          { 'the target group description' }
  ...
  let(:action) {
    WorkCrew::Helpers.create_record(
      ...
      group_desc:    target_group_desc,
      instance_uuid: target_instance_uuid,
      instance_desc: target_instance_desc,
    )
  }

  context 'creates a new WorkCrew::MemberData record' do
    it 'should pass' do
      expect{action}.to change{WorkCrew::MemberData.count}.by 1
    end
